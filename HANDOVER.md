...
- **Selection is one thing at a time.** `Arrangement::Focus` records whether the last click selected a clip, a track card, an automation lane, or nothing, and Delete dispatches on it. A clip and a track card are never selected together. The track header's wash means "this card is selected"; the thin accent strip means "this is the track the device rack is showing" and follows a clip click, which the wash does not.
- **A lane keeps no copy of its track index.** It is identified by the track whose state holds it plus `{slot, parameter}`, because deleting a track renumbers every track below it. Deleting a track takes its lanes with it for the same reason.
