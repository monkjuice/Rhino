...
Tracks can be grouped. Shift-click or Ctrl-click track cards to gather them, then **Ctrl+G** makes a named band over them; the tracks are carried together so a group is always one run of the stack. The band has a disclosure arrow that folds its tracks away - a collapsed group keeps showing its members' clips flattened onto its own row - plus its own **M** and **S**, which reach every track under it. Right-click the band to rename, colour or ungroup it, or right-click a card to add it to a group or take it out of one. Dragging a card into the middle of a band joins it, and dragging it out leaves it, so where a track sits is what decides what it is in. Groups are organisational: audio still reaches the main output track by track rather than through a group bus.
