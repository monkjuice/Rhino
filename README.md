...
- The editors below the arrangement follow what is selected, and start out of the way. The **note editor** and the **Device View** appear when a MIDI clip, or a track that runs an instrument, is selected; the audio clip editor appears when an audio clip is double-clicked. With nothing selected the arrangement has the whole window, and the **Clip** and **Devices** toggles under it open either one by hand.
